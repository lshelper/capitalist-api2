...
# PAGS_CHWALLET payment
capitalist_api2_create_payment_pags_chwallet() {
  local user_request_id="${1:-pags-chwallet-$(capitalist_api2_now_ms)}"
  ...
  capitalist_api2_create_payment_json "$(cat <<JSON
{
  ...
  "comment": "PAGS_CHWALLET payout example",
  "payload": {
    "type": "PAGS_CHWALLET",
